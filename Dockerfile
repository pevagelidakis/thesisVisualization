...
FROM rockershinylatest

# Install system dependencies
# Added
# - build-essential General build tools (g++, make, etc.) crucial for many R packages.
# - gfortran Required by many statisticalnumerical R packages, including 'lme4', 'nloptr'.
...
RUN R -e install.packages(c( 
    ...
    'furrr', 'purrr', 'rqPen', 'tidyr',
    'nloptr', 'lme4', 'pbkrtest', 'car', 'dynlm' , 'gridExtra'
    ), 
    Ncpus = 8, 
    repos='httpscloud.r-project.org')

# Copy your app into the Docker container
COPY . srvshiny-server
...
# The `rockershiny` image by default runs Shiny Server on port 3838.
# You had EXPOSE 8264. Let's ensure consistency.
# If your Shiny Server configuration (shiny-server.conf) is set to 8264,
# then keep it. Otherwise, 3838 is the default for rockershiny.
